'''
    Clase hecha por Matt Zucker y editada por la comunidad para adaptarla de Python 2 a Python 3
    Hemos editado el PAGE_MARGIN_X y PAGE_MARGIN_Y para que se adecue mas a lo que queremos que haga
    Hemos editado el main para que haga lo que no reciba parametros mediantes argumentos, sino por parametros
'''

######################################################################
# OCR_page_dewarp.py - Proof-of-concept of page-dewarping based on a
...
def remap_image(name, img, small, page_dims, params, path):
    ...
    threshfile = path + name + '_thresh.jpeg'
...
def main(filename, path, imagen = None):
    ...
    outfile = remap_image('../threshold/'+name, img, small, page_dims, params, path)
